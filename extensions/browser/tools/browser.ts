...
import { activateBackend, BACKEND_TOOLS, ROUTING_GUIDANCE } from "../routing.ts";
...
const parameters = Type.Object({
  ...
  cdpEndpoint: Type.Optional(
    Type.String({ description: "Explicit local HTTP(S)/WS(S) Chrome CDP endpoint for opt-in shared browser mode." }),
  ),
...
export function registerBrowserTool(pi: ExtensionAPI, runtime: BrowserRuntime): void {
  pi.registerTool({
    ...
    label: "Browser",
    description:
      "Coordinate official Playwright, Chrome DevTools, and Lighthouse CLIs. Prepare one backend for a task, hand off a URL or artifact, compose a report, inspect status, or clean Browser-owned state. Backend tools are loaded on demand.",
    promptSnippet: "Coordinate browser automation, debugging, audits, handoffs, and artifacts",
    promptGuidelines: [...ROUTING_GUIDANCE],
    parameters,
    ...
    async execute(toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (action === "prepare") {
        if (!params.backend)
          throw new Error("browser prepare requires backend: playwright, chrome_devtools, or lighthouse.");
        ...
        if (previous.lastBackend && previous.sharedCdpEndpoint !== cdpEndpoint) {
          ...
          if (closed.failures.length > 0)
            throw new Error(
              closed.failures
                .map((failure) => `Cannot change shared CDP mode; ${failure.target}: ${failure.reason}`)
                .join("\n"),
            );
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: `Prepared ${params.backend}.${cdpEndpoint ? ` Shared CDP enabled for ${cdpEndpoint}.` : ""} Artifact root: ${workspace.root}`,
            },
          ],
          details: {
            action,
            ...
            activated,
            artifactRoot: workspace.root,
            sharedCdpEndpoint: cdpEndpoint,
      ...
      if (action === "handoff") {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Handoff${destination}.${url}${artifact}${cdp} Shared CDP is opt-in and limited to local endpoints.`,
            },
          ],
          details: {
            action,
            backend: params.backend,
            activated: params.backend ? [BACKEND_TOOLS[params.backend]] : [],
            sharedCdpEndpoint: state.sharedCdpEndpoint,
            text: `${state.currentUrl ?? ""}${artifact}`,
          } satisfies BrowserDetails,
        };
      }
      if (action === "report") {
        ...
        return {
          ...
          details: {
            ...
            artifactRoot: (await runtime.workspace(ctx)).root,
          } satisfies BrowserDetails,
        };
      }
      if (action === "close") {
        const closed = await runtime.close(pi, ctx);
        const failureText = closed.failures.map(
          (failure) => `Retained ${failure.target} at ${failure.retainedPath}: ${failure.reason}`,
        );
        const text = [
        ...
        return {
          ...
          details: { action, text, failures: closed.failures } satisfies BrowserDetails,
        };
      }
      if (action === "clear") {
        const closed = await runtime.close(pi, ctx);
        if (closed.failures.length > 0) {
          throw new Error(
            closed.failures
              .map((failure) => `Cannot clear ${failure.target}; retained ${failure.retainedPath}: ${failure.reason}`)
              .join("\n"),
          );
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: deleted.length ? `Deleted current Browser runtime: ${deleted.join(", ")}` : "Nothing to clear.",
            },
          ],
          details: { action, deleted } satisfies BrowserDetails,
        };
      }
