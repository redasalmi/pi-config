...
export async function harness(t: TestContext, liveExec?: ExtensionAPI["exec"]) {
  ...
  const api = {
    ...
    async exec(command: string, args: string[], options?: Parameters<ExtensionAPI["exec"]>[2]): Promise<ExecResult> {
      const backendIndex =
        command === "env" ? args.findIndex((arg) => ["playwright-cli", "chrome-devtools"].includes(arg)) : -1;
      const call = {
        command: backendIndex >= 0 ? args[backendIndex] : command,
        args: backendIndex >= 0 ? args.slice(backendIndex + 1) : args,
        cwd: options?.cwd,
      };
      calls.push(call);
  ...
  const ctx = {
    cwd: directory,
    sessionManager: { getSessionId: () => "browser-tests" },
