...
const execute: ExtensionAPI["exec"] = (command, args, options) =>
...
test("Playwright: JSON/Markdown snapshots, batched workflow, errors, and artifacts", { timeout: 60_000 }, async (t) => {
  ...
  const workflow = await h.invoke("playwright", {
    action: "run_code",
    code: `async page => {
  ...
  await assert.rejects(
    h.invoke("playwright", {
      action: "run_code",
      code: "async page => { throw new Error('synthetic smoke failure'); }",
      json: true,
    }),
...
test(
  "Chrome: warm commands, both error formats, navigation, snapshots, and artifacts",
  { timeout: 60_000 },
  async (t) => {
    const h = await harness(t, execute);
    const url = await fixture(t);
    ...
    for (const outputFormat of ["md", "json"] as const) {
      await assert.rejects(
        h.invoke("chrome_devtools", {
          command: "evaluate_script",
          args: ["() => { throw new Error('synthetic smoke failure'); }"],
          options: { pageId: Number(pageId) },
          outputFormat,
        }),
