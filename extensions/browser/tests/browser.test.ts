...
import { parsePageState } from "../cli/shared.ts";
...
for (const [format, output] of [
  ...
  test(`Chrome rejects ${format} error: ${output}`, async (t) => {
    ...
    await assert.rejects(
      h.invoke("chrome_devtools", { command: "evaluate_script", args: ["() => 1"], outputFormat: format }),
...
test("Chrome checks stderr errors in JSON mode without treating Markdown data as an envelope", async (t) => {
  ...
  h.control.onExec = async (call) =>
    call.args.includes("evaluate_script") ? ok("[]", "Error: synthetic stderr failure") : undefined;
  await assert.rejects(
    h.invoke("chrome_devtools", { command: "evaluate_script", args: ["() => 1"], outputFormat: "json" }),
    /CLI reported an error/,
  );
  h.control.onExec = async (call) =>
    call.args.includes("evaluate_script") ? ok('[{"type":"text","text":"plain data"}]') : undefined;
  ...
  test(`Playwright registers and reads ${mode} snapshots, including with stderr`, async (t) => {
    ...
    const output =
      mode === "markdown"
        ...
        : JSON.stringify(
            mode === "json" ? { snapshot: { file: relative } } : { result: { snapshot: { file: relative } } },
          );
    h.control.onExec = async (call) => (call.args.includes("goto") ? ok(output, "Synthetic warning") : undefined);
    const result = await h.invoke("playwright", {
      action: "goto",
...
test("Playwright JSON snapshot paths cannot escape containment or follow escaping symlinks", async (t) => {
  ...
  h.control.onExec = async (call) =>
    call.args.includes("goto") ? ok(JSON.stringify({ snapshot: { file: outside } })) : undefined;
  ...
  h.control.onExec = async (call) =>
    call.args.includes("goto") ? ok(JSON.stringify({ snapshot: { file: "playwright/link.yml" } })) : undefined;
  await assert.rejects(
    h.invoke("playwright", { action: "goto", url: "https://example.invalid/", json: true }),
    /symlink escape/,
  );
});
...
test("Lighthouse evaluates non-CWV quality and regression thresholds", async (t) => {
  ...
  const params = {
    ...
    thresholds: { "total-byte-weight": 1000 },
    regressionThresholds: { "metrics.total-byte-weight": 100 },
  ...
  await assert.rejects(
    h.invoke("lighthouse_cli", { ...params, regressionThresholds: { "metrics.total-byte-weight": 99 } }),
    /Threshold enforcement failed/,
  );
  await assert.rejects(
    h.invoke("lighthouse_cli", { ...params, thresholds: { "unknown-metric": 1 } }),
    /Threshold enforcement failed/,
  );
...
test("Lighthouse excludes non-finite numeric metrics", async (t) => {
  ...
  const result = await h.invoke("lighthouse_cli", {
    action: "compare_reports",
    baselinePath: path,
    candidatePath: path,
    thresholds: { "total-byte-weight": 1000 },
...
test("Lighthouse medians retain all numeric metrics but keep the display shortlist", async (t) => {
  ...
  const result = await h.invoke("lighthouse_cli", {
    ...
    url: "https://example.invalid/",
    repeatRuns: 3,
    thresholds: { "total-byte-weight": 600 },
    failOnThreshold: true,
  });
  assert.equal(result.details.summary.metricValues["total-byte-weight"], 500);
  assert.equal(result.details.thresholdReport.passed, true);
  assert.deepEqual(
    result.details.summary.metrics.map((item: { id: string }) => item.id),
...
for (const format of ["markdown", "json", "html"] as const) {
  test(`Reports render only ${format} and preserve evidence and artifact provenance`, async (t) => {
    ...
    await h.runtime.recordEvidence(h.ctx, {
      backend: "playwright",
      operation: "run_code",
      status: "passed",
      summary: "<checked>",
    ...
    spy.mock.restore();
    assert.equal(
      directDataSerializations,
      format === "json" ? 0 : 1,
      "Unselected renderers must not serialize evidence",
    );
...
test("Redaction covers assignment-style secrets and never re-redacts its own placeholder", () => {
  for (const input of [
    "session=abc123",
    ...
    assert.match(redacted, /\[REDACTED\]/, `Expected ${input} to be redacted`);
    assert.doesNotMatch(redacted, /abc123/, `Expected ${input} to lose its value`);
  }

  // A space-separated authorization scheme must not shield the credential, and the
  // inserted placeholder must not be matched again as `[REDACTED]]`.
  for (const input of [
    ...
    const redacted = redactSecrets(input);
    assert.doesNotMatch(redacted, /dXNlcjpwYXNzd29yZA==|eyJhbGciOiJIUzI1NiJ9/, `Expected ${input} to be redacted`);
    assert.doesNotMatch(redacted, /\]\]/, `Expected ${input} to keep clean placeholders`);
  }

  assert.equal(redactSecrets("token=[REDACTED]"), "token=[REDACTED]");
  assert.equal(redactSecrets("password=hunter2&user=me"), "password=[REDACTED]&user=me");
  assert.equal(
    redactSecrets("session=abc123 (domain: .example.com) path=/", { cookieValues: true }),
    "session=[REDACTED] (domain: .example.com) path=/",
  );
  assert.equal(redactSecrets(JSON.stringify({ session: "abc123" })), '{\n  "session": "[REDACTED]"\n}');
  // Benign prose that merely mentions a sensitive word keeps its detail.
  assert.equal(redactSecrets("3 open sessions: 2"), "3 open sessions: 2");
  assert.equal(redactSecrets("https://example.com/path?page=2&sort=asc"), "https://example.com/path?page=2&sort=asc");
});

test("Only the Chrome DevTools parser accepts a bare URL line", () => {
  assert.equal(parsePageState("- Page URL: https://example.invalid/").url, "https://example.invalid/");
  assert.equal(parsePageState("- Result URL: https://wrong.invalid/").url, undefined);
  assert.equal(parsePageState("- Result URL: https://wrong.invalid/", { bareUrl: true }).url, "https://wrong.invalid/");
  assert.equal(parsePageState("URL: https://wrong.invalid/").url, undefined);
});

test("Playwright page state ignores non-page URL lines", async (t) => {
  const h = await harness(t);
  h.control.onExec = async (call) =>
    call.args.includes("goto") ? ok("- Page URL: https://example.invalid/\n- Page Title: Example") : undefined;
  const first = await h.invoke("playwright", { action: "goto", url: "https://example.invalid/" });
  assert.equal(first.details.url, "https://example.invalid/");
  assert.equal((await h.runtime.state(h.ctx)).currentUrl, "https://example.invalid/");

  h.control.onExec = async (call) =>
    call.args.includes("goto") ? ok("- Result URL: https://wrong.invalid/\n- Other: keep") : undefined;
  const second = await h.invoke("playwright", { action: "goto", url: "https://example.invalid/" });
  assert.equal(second.details.url, undefined);
  assert.equal((await h.runtime.state(h.ctx)).currentUrl, "https://example.invalid/");
});
