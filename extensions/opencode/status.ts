...
const ISSUES: Record<Issue, string> = {
  ...
  entitlement:
    "The usage endpoint denied Go entitlement. Check that the key belongs to the subscribed workspace member.",
  ...
  "invalid-response":
    "The usage endpoint returned an invalid or oversized response. Cached data has not been replaced.",
...
export function completions(prefix: string) {
  ...
  if (match) {
    ...
    return STATUSLINE_ITEMS.filter((field) => field.startsWith(match[2].slice(split).trim())).map((field) => ({
      value: `${stem}${field}`,
      label: field,
    }));
  }
...
export function registerCommand(
  ...
  settingsChanged: (ctx: ExtensionContext) => void,
): void {
  pi.registerCommand("opencode", {
    ...
    handler: async (args, ctx) => {
      if (!ctx.hasUI) {
        console.error(
          "/opencode informational commands require a TUI or compatible RPC client; no account request made.",
        );
        return;
      }
      ...
      if (command === "warnings") {
        ...
        if (rest.length !== 1 || !["on", "off"].includes(rest[0])) {
          notify(ctx, "Use /opencode warnings on|off.", "error");
          return;
        }
        try {
          state.settings = saveSettings({ warnings: rest[0] === "on" });
        } catch {
          notify(
            ctx,
            "Could not save opencode.json. Check the settings file and directory permissions; no changes applied.",
            "error",
          );
          return;
        }
      ...
      if (command === "statusline") {
        ...
        let fields;
        try {
          fields = parseStatusline(rest.join(" "), state.settings.statusline);
        } catch (error) {
          notify(ctx, (error as Error).message, "error");
          return;
        }
        try {
          state.settings = saveSettings({ statusline: fields });
        } catch {
          notify(
            ctx,
            "Could not save opencode.json. Check the settings file and directory permissions; no changes applied.",
      ...
      notify(
        ctx,
        "Use /opencode usage|status|stats|console|warnings on|off|statusline … . Model selection remains with Pi.",
        "error",
      );
    },
  });
