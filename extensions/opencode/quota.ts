...
export function createQuotaWarnings(state: State) {
  ...
  function observe(ctx: ExtensionContext, snapshot: Snapshot): void {
    ...
    for (const name of WINDOW_NAMES) {
      ...
      if (previous && (Math.abs(window.resetsAt - previous.resetsAt) > 2000 || window.usedPercent < previous.used))
        previous = undefined;
      ...
      if (threshold < (previous?.threshold ?? 100)) {
        notify(
          ctx,
          `OpenCode Go ${WINDOW_LABELS[name]}: ${left}% remaining; ${resetText(window.resetsAt)}. Use /opencode usage for details. Go models share quota; no settings changed.`,
          "warning",
        );
      }
      warned.set(name, {
        ...
        threshold: Math.min(threshold, previous?.threshold ?? 100),
      });
    }
  }
