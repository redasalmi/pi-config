...
import { SERVICE_TIER_REQUEST, type ServiceTierIntegration, type ServiceTierRequest } from "../integration.ts";
import { createPresetsState } from "../state.ts";

type TierModel = Model<any> & { service_tiers?: Array<{ id: string; name: string }> };
...
function findTier(target: Model<any> | undefined, requested: string | undefined): string | undefined {
  const query = requested?.trim();
  if (!query) return undefined;
  const tiers = (target as TierModel | undefined)?.service_tiers ?? [];
  return (
...
function createBaseHarness() {
  ...
  const pi = {
    ...
    registerShortcut(key: string, shortcut: { handler: (ctx: ExtensionCommandContext) => Promise<void> }) {
    ...
    appendEntry(customType: string, data: unknown) {
      entries.push({
        ...
        parentId: entries.length ? String(entries.length - 1) : null,
    ...
    async exec() {
      return { code: 0, stdout: "test-branch\n", stderr: "", killed: false };
    },
  };
  const ctx = {
    cwd: process.cwd(),
    ...
    modelRegistry: {
      ...
      async getProviderAuth() {
        return { auth: { apiKey: "test-only-placeholder" } };
      },
    },
    ui: {
...
// Stand-in tier provider so presets' optional tier contract stays covered.
function createTierIntegration() {
  let active: string | undefined;
  const integration: ServiceTierIntegration = {
    initialize: async () => {},
    get: () => active,
    resolve: (target, requested) => findTier(target, requested),
    set: (_ctx, tier) => {
      active = tier;
    },
  };
...
export function harness(withTierProvider = true) {
  const base = createBaseHarness();
  const state = createPresetsState();
  const tiers = createTierIntegration();
  if (withTierProvider) {
    base.pi.events.on(SERVICE_TIER_REQUEST, (data) => {
      (data as ServiceTierRequest).integration = tiers;
    });
  }
  return {
    ...base,
