...
export function createStatusline(state: CodexState, deps: StatuslineDeps) {
  function renderStatus(ctx: ExtensionContext): boolean {
    ...
    const tier = state.statusline.some((item) => item === "fast" || item === "service-tier")
      ? findServiceTier(ctx.model, state.selectedServiceTier)
      : undefined;
    let stale = false;
    const now = Date.now();
    for (const item of state.statusline) {
      ...
      if (item === "credits") {
        ...
        if (state.resetCreditCount !== undefined && state.resetCreditCount > 0)
          parts.push(itemLabel(ctx, "Resets", String(state.resetCreditCount)));
        if (codex?.credits || state.resetCreditCount !== undefined)
      ...
      if (item === "usage") {
        ...
        if (codex?.spendControlReached || codex?.rateLimitReachedType)
          parts.push(ctx.ui.theme.fg("error", "limit reached"));
  ...
  async function handleStatuslineCommand(args: string, ctx: ExtensionContext): Promise<void> {
    ...
    if (operation === "reset") state.statusline = [...DEFAULT_STATUSLINE];
      ...
      if (unknown.length) {
        notify(ctx, `Unknown items: ${unknown.join(", ")}. Available: ${STATUSLINE_ITEMS.join(", ")}`, "error");
        return;
      }
  ...
  function statuslineCompletions(prefix: string): AutocompleteItem[] | null {
    ...
    return STATUSLINE_ITEMS.filter((item) => item.startsWith(match[2].slice(split).trim())).map((item) => ({
      value: `${stem}${item}`,
      label: item,
    }));
  }
