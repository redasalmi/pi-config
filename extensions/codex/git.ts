...
function requireSuccess(result: GitResult): string {
  if (result.code !== 0 || result.truncated)
    throw new Error(
      result.truncated ? "Git metadata exceeded the output limit" : result.stderr.trim() || "Git command failed",
    );
...
export async function pinReview(cwd: string, base: string, head = "HEAD", signal?: AbortSignal) {
  const pin = async (ref: string) =>
    requireSuccess(await runGit(cwd, ["rev-parse", "--verify", "--end-of-options", `${ref}^{commit}`], signal));
          ...
          if (view === "untracked" && ctx.hasUI && files.length) {
            ...
            if (selected) {
              ...
              if (!stat.isFile() || stat.size > MAX_OUTPUT)
                throw new Error(
                  "Preview supports regular files up to 50 KiB; symlinks and larger files are not opened",
                );
              const preview = await runGit(
                root,
                ["diff", "--no-index", ...diffOptions, "--", "/dev/null", path],
                lifetime.signal,
              );
              if (![0, 1].includes(preview.code) && !preview.truncated)
                throw new Error(preview.stderr || "Preview failed");
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (scope === "working") {
          ...
          if (
            options.some((option) => !/^(base|head)=.+$/.test(option)) ||
          ...
          if (!base) {
            ...
            if (result.code === 0) base = requireSuccess(result);
              ...
              base = await ctx.ui.input("Base ref to review against", "Enter a local base ref", {
                signal: lifetime.signal,
              });
          }
          if (!base) {
          ...
          const skill = pi
            .getCommands()
            .find((command) => command.source === "skill" && command.name === "skill:code-review");
