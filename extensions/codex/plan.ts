...
export function validateSteps(value: unknown): PlanStep[] {
  ...
  const steps = value.map((item) => {
    if (
      ...
      !item.step.trim() ||
      item.step.length > 300 ||
      /[\x00-\x1f\x7f]/.test(item.step) ||
      !STEP_STATUSES.includes(item.status as PlanStep["status"])
    ) {
...
export function createPlanning(pi: ExtensionAPI, state: CodexState): CodexSubcommand {
  ...
  function render(ctx: ExtensionContext): void {
    ...
    ctx.ui.setStatus(
      "codex-plan",
      state.plan.mode === "planning" ? ctx.ui.theme.fg("warning", "PLAN · agent shell/browser blocked") : undefined,
    );
    if (state.plan.mode === "off" || !state.plan.steps.length) {
      ctx.ui.setWidget("codex-plan", undefined);
      return;
    }
    const completed = state.plan.steps.filter((step) => step.status === "completed").length;
    const visible = state.plan.steps
  ...
  function restore(ctx: ExtensionContext): void {
    ...
    const entry = ctx.sessionManager
      .getBranch()
      .reverse()
      .find((entry) => entry.type === "custom" && entry.customType === PLAN_ENTRY_TYPE);
  ...
  pi.registerTool<typeof PLAN_PARAMS, PlanState>({
    ...
    renderResult(result, { expanded }, _theme) {
      ...
      return new Text(
        expanded
          ? details.steps.map((step, index) => `${index + 1}. [${step.status}] ${step.step}`).join("\n")
          : `Checklist: ${completed}/${details.steps.length} completed`,
        0,
        0,
      );
    },
  ...
  return {
    ...
    handler: async (args, ctx) => {
      ...
      if (!ctx.isIdle()) {
        notify(ctx, "Wait for the task to finish before changing planning mode", "warning");
        return;
      }
      if (command === "execute") {
        if (state.plan.mode !== "planning" || !state.plan.steps.length) {
          notify(ctx, "Create a proposed checklist in planning mode first", "warning");
          return;
        }
        if (
          ctx.hasUI &&
          !(await ctx.ui.confirm(
            "Execute this plan?",
            state.plan.steps.map((step, index) => `${index + 1}. ${step.step}`).join("\n"),
          ))
        )
          return;
        ...
        persist(ctx);
        pi.sendUserMessage(
          "Implement the approved session checklist. Respect repository instructions, verification requirements, and separate approvals for destructive or external actions.",
        );
        return;
      }
      ...
      notify(
        ctx,
        state.plan.mode === "planning"
          ? "Planning enabled: agent shell/browser/unknown tools blocked. This is not an OS sandbox; manual commands and other extensions remain your responsibility."
          : `Plan mode: ${state.plan.mode}. No task started.`,
      );
