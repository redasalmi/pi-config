...
let catalogStamp: string | undefined;

async function catalogFileStamp(path: string): Promise<string> {
  try {
    // Nanosecond timestamps keep the cache from missing a rewrite that lands within
    // the same millisecond tick with an unchanged byte length.
    const info = await stat(path, { bigint: true });
    return `${path}:${info.mtimeNs}:${info.ctimeNs}:${info.size}`;
  } catch {
    return `${path}:missing`;
  }
}
...
export async function refreshServiceTierCatalog(
  ...
  const stamp = (await Promise.all(paths.map(catalogFileStamp))).join("|");
  // The catalog is cached by file identity (mtime/ctime/size), so a rewrite that
  // preserves all three within the filesystem's timestamp resolution is not observed.
  // An empty path list means "reset to an empty catalog" and must always reload.
  if (paths.length > 0 && stamp === catalogStamp) return;
