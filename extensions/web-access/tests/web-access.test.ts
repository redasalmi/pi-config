...
test("registers search and fetch with constrained parameter schemas", () => {
  ...
  for (const params of [
    {},
    { query: "" },
    { query: "Pi", numResults: 0 },
    { query: "Pi", numResults: 11 },
    { query: "Pi", numResults: 1.5 },
    { query: "Pi", type: "deep" },
    { query: "Pi", includeDomains: Array(11).fill("example.com") },
    { query: "Pi", excludeDomains: Array(11).fill("example.com") },
  ]) {
...
test("search posts authenticated compact highlights with defaults and omits empty filters", async () => {
  ...
  assert.deepEqual(requests[0].init.headers, {
    ...
    "x-api-key": "test-only-placeholder",
  });
...
for (const [numResults, expected] of [
  [-5, 1],
  [1, 1],
  [7, 7],
  [10, 10],
...
test("search reports no results after filtering unusable URLs", async () => {
  ...
  assert.deepEqual(await harness().invoke("web_search", { query: "Pi" }), {
...
for (const maxAgeHours of [-2, 721, 0.5, NaN, Infinity]) {
  test(`invalid cache age ${maxAgeHours} rejects search and fetch before network access`, async () => {
    ...
    await assert.rejects(
      harness().invoke("web_fetch", { url: "https://example.com", maxAgeHours }),
      /maxAgeHours must be an integer between -1 and 720/,
    );
    assert.equal(requests.length, 0);
  });
}
...
for (const body of [
  ...
  { results: [{ url: null }] },
  { results: [], statuses: [{ status: "pending" }] },
...
test("fetch normalizes URLs, requests compact text, and reports returned URL metadata", async () => {
  respond = () =>
    response({
      results: [{ title: "  Page  ", url: "https://example.com/final", text: "  Readable body  " }],
      statuses: [{ status: "success", error: null }],
    });
  ...
  assert.deepEqual(requests[0].body, {
    ...
    text: { maxCharacters: 10000, verbosity: "compact" },
  });
  assertUntrusted(result.text);
...
for (const maxAgeHours of [-1, 0, 720]) {
  test(`fetch forwards cache policy ${maxAgeHours} and focused highlights instead of full text`, async () => {
    ...
    const result = await harness().invoke("web_fetch", {
    ...
    assert.deepEqual(requests[0].body, {
      ...
      highlights: { query: "What changed?", maxCharacters: 500 },
      maxAgeHours,
    });
    assertUntrusted(result.text);
...
for (const [error, message] of [
  ...
  test(`fetch rejects per-URL failures even if results exist: ${message}`, async () => {
    respond = () =>
      response({
        results: [{ text: "Must not return this" }],
        statuses: [{ status: "success" }, { status: "error", error }],
      });
    await assert.rejects(harness().invoke("web_fetch", { url: "https://example.com" }), {
      message: `Exa could not fetch the requested URL: ${message}`,
    });
  });
}
...
test("fetch distinguishes absent results from absent readable content", async () => {
  const h = harness();
  respond = () => response({ results: [] });
  assert.deepEqual(await h.invoke("web_fetch", { url: "https://example.com" }), {
    text: "No content returned for this URL.",
  ...
  for (const limit of ["bytes", "lines"]) {
    test(`${name} truncates excessive ${limit} and adds a truncation notice`, async () => {
      ...
      const { text } = await harness().invoke(
        name,
        name === "web_search" ? { query: "Pi" } : { url: "https://example.com" },
      );
